...
public class SessionDetailDocs {
    ...
    private static final List<FieldDescriptor> RESPONSE_FIELDS = List.of(
            ...
            fieldWithPath("overallScore").description("종합 점수 (피드백 완료 전이면 null)").type(SimpleType.NUMBER).optional(),
            fieldWithPath("feedbackStatus").description("피드백 상태 (생성 전이면 null)").type(SimpleType.STRING).optional(),
            fieldWithPath("completedAt").description("피드백 완료 시각 (완료 시, ISO-8601)").type(SimpleType.STRING).optional(),
